...
| Manual build | `vitepress`, `vue`, `mermaid` | Static documentation site, local search, and architecture diagrams; development dependencies only. |
...
`optimise storage` probes published npm, Python/pip, uv, Go, pnpm, Bun, NuGet, and safe-category Conda owner-cache rules before adding rows. Bun clears the whole cache (including bunx packages) only after explicit review from a package workspace; NuGet HTTP/global-package clears pin the selected store through one owner environment override and require explicit review. Conda exposes only separately reviewed tarball, index, and logfile category aggregates under one pinned package root; the unused-package category remains beta catalogue only. Custom cache locations outside approved user roots are skipped. With `--allow-beta`, the Yarn adapter accepts only Classic 1.x, rejects repository CLI delegation, pins the reviewed cache path, and requires explicit selection. Modern Yarn remains unsupported. `optimise docker <context>` pins one daemon for Docker-context rules, but no Docker resource rule is published yet. `optimise all` resolves a supplied or saved workdir and a supplied, saved, or first-use discovered Docker context before composing all scopes. No startup-item command is registered.

Filesystem probes use Node/Bun APIs; generated-path deletion uses the isolated system Python standard library. Process execution uses `Bun.spawn`; persistence uses `bun:sqlite`; tests use `bun test`. The CLI has no web runtime or ORM. The separate VitePress manual uses Node.js 22 in CI and deploys through GitHub Pages; see [manual publishing](manual-publishing.md).

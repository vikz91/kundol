...
kundol is a Bun and TypeScript CLI. `src/cli/index.ts` starts Commander and registers `optimise`, read-only `tools` catalogue commands, and the GitHub `issue` link. Storage and project optimisation use published rules from the bundled JSON registry; macOS startup optimisation uses its dedicated service. There is no dashboard or TUI. See [usage](usage.md) and [context](context.md).

```text
CLI registration and selection (src/cli)
  -> registry schema and published rules (src/core/optimisation-registry, registry/)
  -> registry probe/review/apply engine or startup service (src/services/)
...
| `src/services/startup-optimizer/` | Scans launchd and classic Login Items; only eligible user LaunchAgents can be disabled. |
...
The earlier storage, project-optimizer, indexing, scan/clean, and archive modules remain in source but have no public routes in the current flow. The current CLI does not create an archive.

## Safety and error boundaries

Every `optimise` command prints a plan before selection or `-f`. Published registry targets are limited by integration/status, scope, code-approved handlers, and safety tier. The engine re-probes identity and repeats live checks before action; protected inventory has no removal action, and `-f` selects only safe suggestions. Startup apply verifies eligible user LaunchAgent paths. One or more apply failures return exit code 70; scans and audits can also return errors.

The public CLI does not perform broad Docker prune or blanket old-temp deletion. Docker resource inventory and temp ownership checks remain future work. Startup apply still does not re-read a plist label after scanning, and owner-tool cache commands may cause re-downloads. [Context](context.md) records the current limits.
